...
/* Based on
 * https://github.com/j4cbo/j4cDAC/blob/master/driver/libetherdream/etherdream.c
 */
...
static void trace(struct etherdream *d, const char *fmt, ...) {
  ...
  if (d)
    len = snprintf(buf, sizeof buf, "[%d.%06d] %06lx ", (int)(v / 1000000),
                   (int)(v % 1000000), d->dac_id);
  else
    len = snprintf(buf, sizeof buf, "[%d.%06d]        ", (int)(v / 1000000),
                   (int)(v % 1000000));
...
static int wait_for_fd_activity(struct etherdream *d, int usec, int writable) {
  ...
  if (res < 0) log_socket_error(d, "select");
...
static int read_bytes(struct etherdream *d, char *buf, int len) {
  while (d->conn.dc_read_buf_size < len) {
    ...
    res = recv(d->conn.dc_sock, d->conn.dc_read_buf + d->conn.dc_read_buf_size,
...
static void dump_resp(struct etherdream *d) {
  ...
  trace(d, "-- Protocol %d / LE %d / playback %d / source %d\n",
        0 /* st->protocol */, st->light_engine_state, st->playback_state,
        st->source);
  trace(d, "-- Flags: LE %x, playback %x, source %x\n", st->light_engine_flags,
        st->playback_flags, st->source_flags);
...
static int dac_connect(struct etherdream *d) {
  ...
  // See if we have *actually* connected
  {
    ...
    if (getsockopt(conn->dc_sock, SOL_SOCKET, SO_ERROR, (char *)&error, &len) <
        0) {
  ...
  {
    int ndelay = 1;
    if (setsockopt(conn->dc_sock, IPPROTO_TCP, TCP_NODELAY, (char *)&ndelay,
                   sizeof(ndelay)) < 0) {
...
static int check_data_response(struct etherdream *d) {
  struct etherdream_conn *conn = &d->conn;
  if (conn->resp.dac_status.playback_state == 0) conn->dc_begin_sent = 0;
...
static int dac_send_data(struct etherdream *d, struct dac_point *data,
  ...
  if (st->playback_state == 0) {
    ...
    char c = 'p';
    if ((res = send_all(d, &c, sizeof c)) < 0) return res;

    d->conn.pending_meta_acks++;

    /* Block here until all ACKs received... XXX timeout */
    while (d->conn.pending_meta_acks) dac_get_acks(d, 1500);
  ...
  if (st->buffer_fullness > 1600 && st->playback_state == 1 &&
    ...
    struct begin_command b = {
        .command = 'b', .point_rate = (uint32_t)rate, .low_water_mark = 0};
    if ((res = send_all(d, (const char *)&b, sizeof b)) < 0) return res;

    d->conn.dc_begin_sent = 1;
    d->conn.pending_meta_acks++;
  }

  if ((res = dac_get_acks(d, 0)) < 0) return res;
...
#define SHOULD_TRACE()                           \
  (expected_fullness < DEBUG_THRESHOLD_POINTS || \
...
static void *dac_loop(void *dv) {
  ...
  while (1) {
    ...
    while (1) {
      ...
      if (d->conn.resp.dac_status.playback_state != 2) expected_used = 0;

      expected_fullness = d->conn.resp.dac_status.buffer_fullness +
                          d->conn.unacked_points - expected_used;

      /* Now, see how much data we should write. */
      cap = 1700 - expected_fullness;

      if (cap > MIN_SEND_POINTS) break;
      ...
      if (SHOULD_TRACE())
        trace(d,
              "L: st %d om %d; b %d + %d - %d = %d"
              " -> c %d, wait %d us\n",
              d->conn.resp.dac_status.playback_state, d->conn.pending_meta_acks,
              d->conn.resp.dac_status.buffer_fullness, d->conn.unacked_points,
              expected_used, expected_fullness, cap, wait_time);

      microsleep(wait_time);

      if ((res = dac_get_acks(d, 0)) < 0) break;
    }
    ...
    if (SHOULD_TRACE())
      trace(d,
            ...
            d->conn.resp.dac_status.playback_state, d->conn.pending_meta_acks,
            d->conn.resp.dac_status.buffer_fullness, d->conn.unacked_points,
            expected_used, expected_fullness, cap);
    ...
    if (b->repeatcount > 1) {
      ...
      d->frame_buffer_read++;
      if (d->frame_buffer_read >= BUFFER_NFRAMES) d->frame_buffer_read = 0;
...
static void *watch_for_dacs(void *arg) {
  ...
  while (1) {
    ...
    new_dac->dac_id = (buf.mac_address[3] << 16) | (buf.mac_address[4] << 8) |
                      buf.mac_address[5];
...
struct etherdream *etherdream_get(unsigned long idx) {
  ...
  while (d) {
    ...
    if (idx == i || idx == d->dac_id) return d;
    i++;
    d = d->next;
  }
 ...
 * Documented in etherdream.h.
 */
unsigned long etherdream_get_id(int idx) {
  struct etherdream *d = etherdream_get(idx);
  return d->dac_id;
}
 ...
 * Documented in etherdream.h.
 */
const struct in_addr *etherdream_get_in_addr(unsigned long dacId) {
  struct etherdream *d = etherdream_get(dacId);
  return &d->addr;
}

int etherdream_connect(unsigned long dacId) {
  struct etherdream *d = etherdream_get(dacId);
...
int etherdream_is_connected(unsigned long dacId) {
  struct etherdream *d = etherdream_get(dacId);
 ...
 * Documented in etherdream.h.
 */
int etherdream_is_ready(unsigned long dacId) {
  struct etherdream *d = etherdream_get(dacId);
  pthread_mutex_lock(&d->mutex);
 ...
 * Documented in etherdream.h.
 */
int etherdream_wait_for_ready(unsigned long dacId) {
  struct etherdream *d = etherdream_get(dacId);
  pthread_mutex_lock(&d->mutex);
 ...
 * Documented in etherdream.h.
 */
int etherdream_write(unsigned long dacId, const struct etherdream_point *pts,
                     int npts, int pps, int reps) {
  /* Limit maximum frame size */
  if (npts > BUFFER_POINTS_PER_FRAME) npts = BUFFER_POINTS_PER_FRAME;
  ...
  struct buffer_item *next =
      &d->buffer[(d->frame_buffer_read + d->frame_buffer_fullness) %
                 BUFFER_NFRAMES];
  ...
  pthread_mutex_lock(&d->mutex);
  d->frame_buffer_fullness++;
  if (d->state == ST_READY) pthread_cond_signal(&d->loop_cond);
  d->state = ST_RUNNING;
  pthread_mutex_unlock(&d->mutex);
 ...
 * Documented in etherdream.h.
 */
int etherdream_stop(unsigned long dacId) {
  struct etherdream *d = etherdream_get(dacId);
  pthread_mutex_lock(&d->mutex);
  if (d->state == ST_RUNNING) d->buffer[d->frame_buffer_read].repeatcount = 0;
  pthread_mutex_unlock(&d->mutex);
  return 0;
}

void etherdream_disconnect(unsigned long dacId) {
  struct etherdream *d = etherdream_get(dacId);
  trace(d, "L: Disconnecting.\n");

  pthread_mutex_lock(&d->mutex);
  if (d->state == ST_READY) pthread_cond_broadcast(&d->loop_cond);
  d->state = ST_SHUTDOWN;
  pthread_mutex_unlock(&d->mutex);
